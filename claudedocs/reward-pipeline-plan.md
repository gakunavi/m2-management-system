...
| 報酬の確定タイミング | 案件が**受注/成果確定ステータス**になった時点（既存の売上計上 `revenueRecognition` と同基準） |
...
3. ⚠️**要確認**: `calculateMonthlyRevenue` は「現在ちょうど受注ステータス」で絞る（exact match）。案件が受注→納品完了等に**進むと報酬がライブ計算から落ちる**。受注が終端なら問題なし。post-受注ステータスがある事業では「受注に到達したか」で判定する必要（`projectStatusChangedAt` 併用）。→ Phase 1 前にユーザーの運用確認が必須
