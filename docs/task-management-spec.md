...
- ステータス / 優先度（セレクト）
- 担当者 / 期限（「なし」ボタンでクリア可能）
- タグ（**共通タグチップ一覧** + 入力+サジェスト+新規作成）
- 説明（テキストエリア）
- 手順チェック ☑（追加/チェック/削除、進捗バー表示）
- 子タスク 📋（一覧+追加、進捗バー表示、**クリックで子タスク詳細に遷移**）
- 通知設定（レベル+通知先）
- メモ（備考）欄
- 削除ボタン
...
| **1.1** | タグチップ一覧選択 + チェックリスト/サブタスクUI明確化 + ツリー表示 + パンくず遷移 | 未着手 |
| **1.5** | タスクボード（DB + API + ボードタブUI + メンバー管理） | 未着手 |
...
Step B: 手順チェック vs 子タスクの明確化
  - task-checklist.tsx: ラベルを「手順チェック ☑」に変更、アイコン追加
  - task-subtasks.tsx: ラベルを「子タスク 📋」に変更、アイコン追加
  - task-detail-panel.tsx: セクション間のセパレータ強化
