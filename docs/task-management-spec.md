...
| サブタスク | 階層型サブタスク（2階層まで）+ 手順チェック 両方 |
| 優先度 | 緊急・高・中・低の4段階 |
...
- テーブル形式: No. / タスク名 / ステータス / 優先度 / 担当者 / 期限 / タグ / アーカイブ / 更新日
- ヘッダークリックでソート切替
...
- **ドラッグ&ドロップ**: 行の並び替え + タスクを親の上にドロップでサブタスク化 / サブタスクを外にドロップで独立化
- ページネーション
...
| **2** | リストD&D並び替え + D&Dサブタスク化/昇格 + カンバンビュー + カレンダービュー | 未着手 |
...
### Phase 2 実装ステップ（D&D + カンバン + カレンダー）

```
Step A: リストビューのドラッグ&ドロップ並び替え
  - dnd-kit SortableContext でタスク行をドラッグ可能に
  - sortOrder をドロップ位置で更新（reorder API）
  - DragOverlay でドラッグ中のプレビュー表示

Step B: D&Dでサブタスク化 / 独立タスク昇格
  - タスクを親タスクの上にドロップ → parentTaskId を設定（サブタスク化）
  - サブタスクを一覧エリアにドロップ → parentTaskId を null に（独立化）
  - 制限: 子タスクを持つタスクはサブタスク化不可 / 2階層制限
  - 視覚フィードバック: ドロップ先がサブタスク化エリアの場合はインデント表示

Step C: カンバンビュー
  - 4列レイアウト: 未着手 / 進行中 / 保留 / 完了
  - dnd-kit で列間ドラッグ&ドロップ（ステータス変更）
  - DragOverlay でカード表示
  - カード: タイトル、担当者、期限、優先度バッジ、サブタスク進捗、タグ
  - reorder API でステータス + sortOrder 更新
