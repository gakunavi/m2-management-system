...
interface PartnerBusinessLink {
  ...
  businessParentName: string | null;
  businessParentCode: string | null;
  createdAt: string;
  updatedAt: string;
}
...
export function PartnerBusinessLinksTab({ entityId }: Props) {
  ...
  const { data: links = [], isLoading } = useQuery<PartnerBusinessLink[]>({
    queryKey: ['partner-business-links', entityId],
    queryFn: async () => {
      const res = await fetch(`/api/v1/partners/${entityId}/business-links`);
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['partner-business-links', entityId] });
  ...
  const updateMutation = useMutation({
    mutationFn: async ({
      ...
      const res = await fetch(`/api/v1/partners/${entityId}/business-links/${linkId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleSelectParent = (link: PartnerBusinessLink, parentId: number) => {
    updateMutation.mutate({
      linkId: link.id,
      data: { businessParentId: parentId },
    });
  };

  // 1次代理店（親なし）として設定
  const handleSetAsPrimary = (link: PartnerBusinessLink) => {
    updateMutation.mutate({
      linkId: link.id,
      data: { businessTier: '1次代理店', businessParentId: null },
    });
  };

  // 階層設定をクリア（未設定に戻す）
  const handleClearHierarchy = (link: PartnerBusinessLink) => {
    updateMutation.mutate({
      linkId: link.id,
      data: { businessTier: null, businessParentId: null },
    });
  };
  ...
  return (
    <div className="space-y-4">
                ...
                <TableRow key={link.id}>
                  ...
                  <TableCell>
                    <Select
                      ...
                      onValueChange={(val) =>
                        updateMutation.mutate({ linkId: link.id, data: { linkStatus: val } })
                      }
                    >
                  ...
                  {/* 担当者/窓口 */}
                  <TableCell>
                    {editingField?.linkId === link.id && editingField.field === 'contactPerson' ? (
                      <Input
                        value={editValue}
                        onChange={(e) => setEditValue(e.target.value)}
                        onBlur={commitEdit}
                      ...
                      <button
                        className="text-sm hover:underline cursor-pointer text-left"
                        onClick={() => startEditing(link.id, 'contactPerson', link.contactPerson)}
                      >
                        {link.contactPerson || '-'}
                      </button>
                    )}
                  ...
                  {/* 事業別親代理店（メインの操作列） */}
                  <TableCell>
                    <BusinessParentPartnerSelect
                      businessId={link.businessId}
                      excludePartnerId={entityId}
                      value={link.businessParentId}
                      valueName={link.businessParentName}
                      valueCode={link.businessParentCode}
                      currentTier={link.businessTier}
                      onSelectParent={(parentId) => handleSelectParent(link, parentId)}
                      onSetAsPrimary={() => handleSetAsPrimary(link)}
                      onClear={() => handleClearHierarchy(link)}
                    />
                  </TableCell>
      ...
      <ConfirmModal
        ...
        title="事業リンクを解除しますか？"
        description="この操作は元に戻せません。事業別の階層情報も削除されます。"
        confirmLabel="解除する"
        variant="destructive"
        onConfirm={() => {
