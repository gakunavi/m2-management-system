...
export interface InvoiceUploadDialogProps {
  partnerId: number;
  businesses: Business[];
  open: boolean;
...
export function InvoiceUploadDialog({
  ...
  useEffect(() => {
    if (open && editingDocument) {
      setBusinessId(String(editingDocument.businessId));
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEditing) {
        url = `/api/v1/partners/${partnerId}/documents/${editingDocument.id}`;
        method = 'PUT';
        if (selectedFile) {
          formData.append('file', selectedFile);
        }
      } else {
        url = `/api/v1/partners/${partnerId}/documents`;
        method = 'POST';
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-md max-h-[90vh] flex flex-col">
        <DialogHeader>
          <DialogTitle>
            {isEditing ? '支払明細書を編集' : '支払明細書をアップロード'}
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4 overflow-y-auto flex-1 min-h-0">
          ...
          <div className="space-y-1.5">
            ...
            >
              <SelectTrigger>
                <SelectValue placeholder="事業を選択してください" />
              </SelectTrigger>
              <SelectContent>
                {businesses.map((b) => (
          ...
          {/* タイトル（必須） */}
          <div className="space-y-1.5">
            <Label htmlFor="invoice-title">タイトル</Label>
            <Input
              id="invoice-title"
              value={documentTitle}
              onChange={(e) => setDocumentTitle(e.target.value)}
              placeholder="例: 2026年2月 支払明細書"
            />
          </div>

          {/* 対象年月（必須） */}
          <div className="space-y-1.5">
            <Label htmlFor="invoice-target-month">対象年月</Label>
            <Input
              id="invoice-target-month"
              type="month"
              value={targetMonth}
          ...
          {/* 説明（任意） */}
          <div className="space-y-1.5">
            <Label htmlFor="invoice-description">説明（任意）</Label>
            <Textarea
              id="invoice-description"
              value={description}
              onChange={(e) => setDescription(e.target.value)}
              placeholder="補足事項を入力してください"
              rows={2}
            />
          </div>
        </div>
