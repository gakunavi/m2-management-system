...
} from './partner-contact-form-modal';
...
export function PartnerContactsTab({ entityId }: Props) {
  ...
  const { data: contacts = [], isLoading } = useQuery<ContactRecord[]>({
    queryKey: ['partner-contacts', entityId],
    queryFn: async () => {
      const res = await fetch(`/api/v1/partners/${entityId}/contacts`);
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['partner-contacts', entityId] });
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: number; data: ContactFormData }) => {
      const res = await fetch(`/api/v1/partners/${entityId}/contacts/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          ...data,
  ...
  const deleteMutation = useMutation({
    mutationFn: async (contactId: number) => {
      const res = await fetch(`/api/v1/partners/${entityId}/contacts/${contactId}`, {
        method: 'DELETE',
      });
