...
export const partnerListConfig: EntityListConfig = {
  ...
  columns: [
    // ── 基本情報 ──
    { key: 'partnerCode', label: '代理店コード', width: 130, sortable: true, group: '基本情報' },
    {
      key: 'partnerName',
      label: '代理店名',
      minWidth: 200,
    ...
    { key: 'partnerSalutation', label: '呼称', width: 150, sortable: true, group: '基本情報', edit: { type: 'text', placeholder: '例：〇〇商事' } },
    { key: 'partnerTierNumber', label: '階層番号', width: 100, sortable: true, group: '基本情報' },
    { key: 'partnerTier', label: '階層', width: 130, sortable: true, group: '基本情報' },
    { key: 'parentPartnerName', label: '親代理店', width: 180, sortable: false, defaultVisible: false, group: '基本情報', render: (value) => value ? String(value) : '-' },
    { key: 'partnerType', label: '種別', width: 110, sortable: true, group: '基本情報', edit: { type: 'select', options: PARTNER_TYPE_OPTIONS } },
    // ── 担当者情報 ──
    {
    ...
    // ── 連絡先 ──
    { key: 'partnerPostalCode', label: '郵便番号', width: 110, sortable: true, defaultVisible: false, group: '連絡先', edit: { type: 'text', placeholder: '000-0000' } },
    { key: 'partnerAddress', label: '住所', minWidth: 200, sortable: true, defaultVisible: false, group: '連絡先', edit: { type: 'text', placeholder: '都道府県・市区町村・番地' } },
    { key: 'partnerPhone', label: '電話番号', width: 140, sortable: true, group: '連絡先', edit: { type: 'phone', placeholder: '03-0000-0000' } },
    { key: 'partnerFax', label: 'FAX', width: 140, sortable: true, defaultVisible: false, group: '連絡先', edit: { type: 'phone', placeholder: '03-0000-0000' } },
    { key: 'partnerEmail', label: 'メール', width: 200, sortable: true, defaultVisible: false, group: '連絡先', edit: { type: 'email', placeholder: 'info@example.com' } },
    { key: 'partnerWebsite', label: 'Webサイト', width: 120, sortable: true, defaultVisible: false, group: '連絡先', edit: { type: 'url', placeholder: 'https://example.com' } },
    // ── 企業情報 ──
    {
      key: 'industryId',
      label: '業種',
    ...
    { key: 'partnerEstablishedDate', label: '設立日', width: 130, sortable: true, defaultVisible: false, group: '企業情報', edit: { type: 'date' } },
    { key: 'partnerCorporateNumber', label: '法人番号', width: 150, sortable: true, defaultVisible: false, group: '企業情報', edit: { type: 'text' } },
    { key: 'partnerInvoiceNumber', label: 'インボイス番号', width: 160, sortable: true, defaultVisible: false, group: '企業情報', edit: { type: 'text' } },
    {
      key: 'partnerCapital',
      label: '資本金',
      width: 140,
    ...
    // ── その他 ──
    { key: 'partnerBpFormUrl', label: 'BP申込書', width: 120, sortable: false, defaultVisible: false, group: 'その他', render: (v) => (v ? 'あり' : '-') },
    { key: 'partnerFolderUrl', label: 'フォルダURL', width: 120, sortable: true, defaultVisible: false, group: 'その他', edit: { type: 'url', placeholder: 'https://drive.example.com/...' } },
    ...
    // ── 手数料情報（選択中の事業デフォルト+リンク上書きをマージした実効値。事業未選択時は "-"） ──
    { key: 'rewardShotDirect', label: 'ショット手数料（担当代理店）', width: 150, align: 'right', defaultVisible: false, group: '手数料情報', render: (v) => (v as string | null) ?? '-' },
    { key: 'rewardShotIndirect', label: 'ショット手数料（上位代理店）', width: 150, align: 'right', defaultVisible: false, group: '手数料情報', render: (v) => (v as string | null) ?? '-' },
    { key: 'rewardStockDirect', label: 'ストック手数料（担当代理店）', width: 150, align: 'right', defaultVisible: false, group: '手数料情報', render: (v) => (v as string | null) ?? '-' },
    { key: 'rewardStockIndirect', label: 'ストック手数料（上位代理店）', width: 150, align: 'right', defaultVisible: false, group: '手数料情報', render: (v) => (v as string | null) ?? '-' },
    // ── システム ──
  ...
  csv: {
    ...
    templateColumns: [
      ...
      { key: 'partnerCorporateNumber', label: '法人番号', description: '自由記述', example: '1234567890123' },
      { key: 'partnerInvoiceNumber', label: 'インボイス番号', description: '自由記述', example: 'T1234567890123' },
...
export const partnerFormConfig: EntityFormConfig = {
  ...
  sections: [
    ...
    },
    {
      ...
      columns: 2,
      fields: [
        { key: 'partnerPostalCode', label: '郵便番号', type: 'text', placeholder: '000-0000', addressAutoFill: { targetKey: 'partnerAddress' } },
    ...
    },
    {
      ...
      fields: [
        ...
        { key: 'partnerEstablishedDate', label: '設立日', type: 'date' },
        { key: 'partnerCorporateNumber', label: '法人番号', type: 'text' },
        { key: 'partnerInvoiceNumber', label: 'インボイス登録番号', type: 'text' },
  ...
  validationSchema: z.object({
    ...
    partnerCorporateNumber: z.string().max(100, '法人番号は100文字以内で入力してください').optional().nullable().or(z.literal('')),
    partnerInvoiceNumber: z.string().max(100, 'インボイス番号は100文字以内で入力してください').optional().nullable().or(z.literal('')),
